...
import { CALCULATORS_REGISTRY, CALCULATOR_CATEGORIES, CalculatorItem } from '@/data/calculators';
...
export default function SearchHero() {
  ...
  const filteredCalculators = CALCULATORS_REGISTRY.filter((item) => {
    const matchesSearch =
  ...
  return (
    <section className="relative pt-12 pb-16 bg-gradient-to-b from-sky-50/70 via-white to-slate-50 border-b border-slate-200/60 overflow-hidden">
      ...
      <div className="absolute top-0 left-1/2 -translate-x-1/2 w-full max-w-7xl h-96 pointer-events-none overflow-hidden">
        ...
        <div className="absolute top-10 right-1/4 w-96 h-96 bg-indigo-100/50 rounded-full blur-3xl" />
      </div>

      <div className="relative max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
        
        {/* Badge */}
        <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full text-xs font-semibold bg-sky-100/80 text-sky-800 border border-sky-200/60 mb-6 shadow-sm">
        ...
        <p className="text-base sm:text-lg text-slate-600 max-w-2xl mx-auto mb-8 font-normal leading-relaxed">
          Simuladores financeiros, rescisões trabalhistas, atualização de valores e regras previdenciárias. Respostas rápidas e confiáveis sem complicações.
        </p>
        ...
        <div className="relative max-w-2xl mx-auto mb-8">
          <div className="relative flex items-center">
            ...
            <input
              ...
              placeholder="Digite o que deseja calcular (ex: Juros compostos, Rescisão, IPCA-E)..."
              className="w-full pl-12 pr-4 py-4 rounded-2xl bg-white border border-slate-300 text-slate-900 placeholder:text-slate-400 font-medium text-base shadow-lg shadow-slate-200/50 focus:outline-none focus:ring-2 focus:ring-sky-500 focus:border-transparent transition-all"
            />
            {searchTerm && (
              <button
        ...
        {searchTerm || selectedCategory !== 'all' ? (
          <div className="text-left bg-white rounded-2xl border border-slate-200 p-6 shadow-xl max-w-4xl mx-auto">
            <h2 className="text-sm font-bold uppercase tracking-wider text-slate-400 mb-4">
              Resultados encontrados ({filteredCalculators.length})
            </h2>
            {filteredCalculators.length > 0 ? (
                ...
                {filteredCalculators.map((calc) => {
                  ...
                  return (
                    <Link
                      ...
                      className="p-4 rounded-xl border border-slate-200/80 bg-slate-50/50 hover:bg-sky-50/50 hover:border-sky-300 transition-all group flex items-start gap-3.5"
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <h3 className="font-bold text-slate-900 group-hover:text-sky-600 transition-colors text-base truncate">
                          {calc.name}
                        </h3>
                      ...
                      <ArrowRight className="w-4 h-4 text-slate-400 group-hover:text-sky-600 group-hover:translate-x-1 transition-all self-center" />
                    </Link>
                  );
                })}
              </div>
              ...
              <p className="text-sm text-slate-500 text-center py-8">
                Nenhuma calculadora encontrada para &quot;{searchTerm}&quot;. Experimente outros termos como juros, rescisão ou aposentadoria.
              </p>
            )}
          </div>
        ) : null}
